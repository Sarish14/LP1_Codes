...
public class pass_1 {
	...
	public static void main(String[] args) throws Exception{
	    ...
	    {
	        String parts[] = line.split("\\s+");        //split("\\s+") will split the string into string of array with separator as space or multiple spaces. \s+ is a regular expression for one or more spaces.
	        
	        if(parts[0].equalsIgnoreCase("MACRO")==true)
	        {
	            flag = 1;
	            ...
	            for(int i=1; i<parts.length; i++) //processing of parameters
	            {
	                ...
	                if(parts[i].contains("="))
	                {
	                    ...
	                    if(keywordParam.length == 2)										// if it has default argument
	                    {
